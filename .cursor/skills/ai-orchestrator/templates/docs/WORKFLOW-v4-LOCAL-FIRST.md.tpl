...
1. **先 push** `docs/progress.md`，再發 `[status]`（內容一致）。
2. **Cursor IDE** 用 Slack MCP `slack_read_channel` / `slack_search_public` 驗證鏡像。
3. **Slack 內 bot** 不會自動訂閱頻道；需 **人在 progress thread @** 或 bot 產品內建的頻道歷史權限。
4. 建議 **Pin 進度串**，`[status]` 以 `thread_ts` 回覆；`docs/progress.md` 記錄 `Slack progress thread`。
5. 唯讀摘要用 `[read-only]`（見 `slack-handoff-template.md` §6），**不是** `[handoff]`。

Skill 詳述：`.cursor/skills/ai-orchestrator/slack-status-sync.md`
