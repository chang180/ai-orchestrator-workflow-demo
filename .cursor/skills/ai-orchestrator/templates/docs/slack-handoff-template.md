...
- `slack_read_channel` → 最新訊息含本則 `[status]`
- `slack_search_public` → `query="[status] in:#{{SLACK_PROJECT_CHANNEL}}"`

**建議**：Pin 一則「進度串」後，後續 `[status]` 用 `thread_ts` 回覆同一串（見 §6）。
...
## 6. Progress thread 與 `[read-only]`（讓 Slack 內 AI 讀進度）
...
本串為 {{PRODUCT_NAME}} 進度鏡像專用。之後 [status] 請回覆在本串。
SoT: docs/progress.md · Repo: {{GITHUB_FULL_REPO}}
```

→ **Pin** → 將 `thread_ts` 寫入 `docs/progress.md`。

### 6b. 在進度串發 [status]

`slack_send_message` 帶 `thread_ts=<progress thread>`，正文同 §1。
